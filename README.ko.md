...
이 패키지는 **단독으로 완결**됩니다 — 업스트림 (flutter_svg 본체) 의
지원을 전제하지 않습니다. 장래에 flutter_svg가 writing-mode를 네이티브
지원해도 충돌하지 않습니다 (전처리 후의 SVG에는 writing-mode가 남지
않기 때문).

## 이 패키지의 은퇴

flutter_svg가 font-family 목록 해석과 세로쓰기를 모두 지원하게 되면,
이 패키지는 역할을 마치고 은퇴합니다. 파생 패키지는 업스트림이
따라올 때까지의 다리이며, 건넌 다리는 더 이상 필요 없습니다.
그때까지는 단독으로 동작하고, 은퇴 후에도 최종판은 그대로 사용할 수
있습니다.
